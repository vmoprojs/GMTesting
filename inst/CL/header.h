...
double CorFunBohman_t(double lag, double scale);
double CorFunBohman_st(double lag, double scale, double lag1, double scale1);
...
void integr_gen_aux(double *x, int n, void *ex);
void L220(double sum, double *bi);
void L230(double sum, double *bi, double nu, double *x, double aa, int *nb, int n, int *ize);
void L120(double sum, double *bi, double nu, double *x, double aa, int *nb, int n, int *ize, int nend, double p, double em, double en, double emp2al, int l, double twonu, double cc, double empal, double bb);
void L90(double sum, double *bi, double nu, double *x, double aa, int *nb, int n, int *ize, int nend, double p, double em, double en, double emp2al, int l, double twonu, double cc, double empal, double bb, int *ncalc);
void L420(int i, int *ncalc, int *nb, double *bk);
void L140(int *neval, int *last);
void L130(int *neval, int *last, int *ier);
void L115(double *result, double *abserr, int k, int *last, double *rlist, double errsum);
void L110(int *neval, int *last, int *ier, double *result, int ksgn, double area, double defabs, double errsum);
void L901();
void L100(double *abserr, double epmach, double *result);
void L50(int limexp, int *n, int num, int newelm, int ib, int ie, int i__, int ib2, double *epstab, int indx);
void L30(double res, double e1, double e2, double ss, double *epstab, int k1, double errA, double err2, double err3, double *abserr, double *result);
void Last(int *maxerr, int *iord, int *nrmax, double *ermax, double *elist);
...
double bessel_ii(double x, double alpha, double expo)
{
	...
	if (alpha < 0) {
		...
		return(bessel_jj(x, -alpha, expo) +
			((alpha == na) ? 0 :
				bessel_ii(x, -alpha, expo) *
	...
	iord, int *last)
{
	...
	*abserr = oflow;
	...
	for (*last = 2; *last <= *limit; ++(*last)) {
		...
		if (errsum <= errbnd) { L115(result,abserr,k,last,rlist,errsum); }// ===jump out of do-loop
		if (*ier != 0) { break; }
		if (*last == 2) { // L80:
			small = fabs(*b - *a) * .375;
			erlarg = errsum;
	...
	if (*abserr == oflow) { L115(result, abserr, k, last, rlist, errsum);; }
	if (*ier + ierro == 0) { L110(neval,last,ier,result, ksgn,area,defabs,errsum); }
	if (ierro == 3)
		*abserr += correc;
	if (*ier == 0)
		*ier = 3;
	if (*result == 0. || area == 0.) {
		if (*abserr > errsum) { L115(result, abserr, k, last, rlist, errsum);; }
		if (area == 0.) { L130(neval,last,ier); }
	}
	else { // L105:
		if (*abserr / fabs(*result) > errsum / fabs(area))
		{
			L115(result, abserr, k, last, rlist, errsum);;
		}
	}
	L130(neval, last, ier);
	...
	for (i__ = 1; i__ <= newelm; ++i__) {
		...
		if (err2 <= tol2 && err3 <= tol3) {
			...
			*abserr = err2 + err3;//	abserr = fabs(e1-e0)+fabs(e2-e1)
			...
			L100(abserr,epmach,result);	// ===jump out of do-loop
		}
		...
		if (err1 > tol1 && err2 > tol2 && err3 > tol3) {
			...
			if (epsinf > 1e-4) {
				L30(res,e1,e2,ss,epstab,k1,errA,err2,err3,abserr,result);
			}
		}

		*n = i__ + i__ - 1;
		L50(limexp,n,num,newelm,ib,ie,i__,ib2,epstab,indx);// ===jump out of do-loop
	}
	...
	double *abserr, double *resabs, double *resasc)
{
	...
	//f(vec, 21, ex);
	integr_gen_aux((double*)vec, 21, ex);
	fc = vec[0];
	resk = wgk[10] * fc;
	*resabs = fabs(resk);
	for (j = 1; j <= 5; ++j) {
	...
	double *ermax, double *elist, int *iord, int *nrmax)
{
	...
	if (*last <= 2) {
		iord[1] = 1;
		iord[2] = 2;
		Last(maxerr, iord, nrmax, ermax, elist);
	}
...
double CorFunBohman_st(double lag, double scale,double lag1, double scale1)
{
	...
	double rho1 = 0.0, x1 = 0;
	x1 = lag1 / scale1;
	if (x1 <= 1)
	{
		if (x1>0) rho1 = (1 - x1)*(sin(2 * M_PI*x1) / (2 * M_PI*x1)) + (1 - cos(2 * M_PI*x1)) / (2 * M_PI*M_PI*x1);
		else   rho1 = 1;
	}
	else rho1 = 0;
